...
def driverview(started: bool, params: Params, CP: car.CarParams, classic_model, tinygrad_model, frogpilot_toggles) -> bool:
  # Road/driver camera process: run whenever the car is started, or when driver view is explicitly enabled.
  return started or params.get_bool("IsDriverViewEnabled")

def dm_enabled(started: bool, params: Params, CP: car.CarParams, classic_model, tinygrad_model, frogpilot_toggles) -> bool:
  # Respect FrogPilot's Disable Driver Monitoring toggle: when enabled, do not run DM model/daemon
  if getattr(frogpilot_toggles, "disable_driver_monitoring", False):
    return False
  # Otherwise follow the same policy as driver view
  return started or params.get_bool("IsDriverViewEnabled")
...
def ublox(started, params, CP: car.CarParams, classic_model, tinygrad_model, frogpilot_toggles) -> bool:
  # Allow GM-specific toggle to completely disable GPS hardware daemons
...
def qcomgps(started, params, CP: car.CarParams, classic_model, tinygrad_model, frogpilot_toggles) -> bool:
  # When GMDisableGps is enabled for GM cars, do not start the Qualcomm GPS daemon either
  if CP.carName == "gm" and getattr(frogpilot_toggles, "gm_disable_gps", False):
    return False
...
procs = [
  ...
  PythonProcess("dmonitoringmodeld", "selfdrive.modeld.dmonitoringmodeld", dm_enabled, enabled=(not PC or WEBCAM)),
  ...
  PythonProcess("dmonitoringd", "selfdrive.monitoring.dmonitoringd", dm_enabled, enabled=(not PC or WEBCAM)),
